...
/** Stores the puzzle and provides functions for querying the puzzle. */
public class Puzzle implements Serializable {
	/** <p>The puzzle is a 2 dimensional array of atomic integers encoded as an AtomicIntegerArray.
	 * The upper-left, or north-western corner of the puzzle is
	 * given by puzzle.get(0).  The lower-right, or south-eastern corner of the puzzle is
	 * puzzle.get(puzzle.length()-1).  The puzzle is stored in row-major order, so the element at
	 * row n, column m is given by puzzle.get(n * width + m).</p>
	 * 
	 * <p>The entrance to the puzzle is at row: 0, column: width/2.
	 * The exit is at row: height-1, column: width/2.</p>
	 * 
	 * <p>Individual bits of each byte in the "puzzle" array are used to encode
	 * information about that cell of the puzzle.  Only the low-order 8 bits
	 * are used.  The designated bits are:</p>
	 * <pre>  CCSSEEse</pre>
	 * <p>and have the following meaning</p>
	 * <dl>  <dt><pre>CC</pre> <dd>marks two bits that are used to give a color to this cell in the puzzle.  Each of the four
	 *      bit combinations corresponds to a different color that will be used by the PuzzleDisplay object
	 ...
	 * 
	 * <p>There is always a wall to the south of cells on the southern border of the puzzle.
	 * There is always a wall to the east of cells on the eastern border of the puzzle.
	 * There is an implicit wall to the north of cells on the northern border
	 *   and to the west of cells on the western border.</p> */
	public AtomicIntegerArray puzzle;
	public int width, height;
	
	/**
	 * A reference to the graphical display of this puzzle.  This will be null if graphical
	 * puzzle display is not currently enabled.
	 */
	public transient PuzzleDisplay display;
	 ...
	 * described in the documentation for the {@link Puzzle#puzzle puzzle} field. */
	public int getCell(Position pos) {
		return puzzle.get(pos.col + pos.row * width);
	}
	
	private void setCell(Position pos, int val) {
		puzzle.set(pos.col + pos.row * width, val);
	}
	
	boolean condSetCell(Position pos, int oldVal, int newVal) {
		return puzzle.compareAndSet(pos.col + pos.row * width, oldVal, newVal);
	}
	...
	/** Returns the position corresponding to the entrance of the puzzle. */
	public Position getStart() {
		return new Position(getWidth()/2, 0);
	}

	/** Returns the position corresponding to the exit of the puzzle. */
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
		width = in.readInt();
		height = in.readInt();
		puzzle = new AtomicIntegerArray(width * height);
		Position pos = new Position(0,0);
		while(pos.row < height) {
